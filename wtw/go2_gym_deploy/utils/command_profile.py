...
class ConstantAccelerationProfile(CommandProfile):
    def __init__(self, dt, max_speed, accel_time, zero_buf_time=0):
        ...
        self.commands[zero_buf_timesteps : zero_buf_timesteps + accel_timesteps, 0] = (
            torch.arange(0, max_speed, step=max_speed / accel_timesteps)
        )
...
class ElegantForwardProfile(CommandProfile):
    def __init__(
        self, dt, max_speed, accel_time, duration, deaccel_time, zero_buf_time=0
    ):
        import numpy as np
        ...
        x_vel_cmds = (
            [0] * zero_buf_timesteps
            + [*np.linspace(0, max_speed, accel_timesteps)]
            + [max_speed] * duration_timesteps
            + [*np.linspace(max_speed, 0, deaccel_timesteps)]
        )
...
class ElegantYawProfile(CommandProfile):
    def __init__(
        self, dt, max_speed, zero_buf_time, accel_time, duration, deaccel_time, yaw_rate
    ):
        import numpy as np
...
class ElegantGaitProfile(CommandProfile):
    def __init__(self, dt, filename):
        ...
        self.commands[:len_command_sequence, 0] = torch.Tensor(
            command_sequence["x_vel_cmd"]
        )
        self.commands[:len_command_sequence, 2] = torch.Tensor(
            command_sequence["yaw_vel_cmd"]
        )
        self.commands[:len_command_sequence, 3] = torch.Tensor(
            command_sequence["height_cmd"]
        )
        self.commands[:len_command_sequence, 4] = torch.Tensor(
            command_sequence["frequency_cmd"]
        )
        self.commands[:len_command_sequence, 5] = torch.Tensor(
            command_sequence["offset_cmd"]
        )
        self.commands[:len_command_sequence, 6] = torch.Tensor(
            command_sequence["phase_cmd"]
        )
        self.commands[:len_command_sequence, 7] = torch.Tensor(
            command_sequence["bound_cmd"]
        )
        self.commands[:len_command_sequence, 8] = torch.Tensor(
            command_sequence["duration_cmd"]
        )
    ...
    def get_command(self, t, probe=False):
        ...
        for button in range(4):
            if self.triggered_commands[button] is not None:
                ...
                if (
                    self.currently_triggered[button]
                    and t < self.triggered_commands[button].max_timestep
                ):
...
class RCControllerProfileAccel(RCControllerProfile):
    def __init__(self, dt, state_estimator, x_scale=1.0, y_scale=1.0, yaw_scale=1.0):
        super().__init__(
            dt, state_estimator, x_scale=x_scale, y_scale=y_scale, yaw_scale=yaw_scale
        )
        self.x_scale, self.y_scale, self.yaw_scale = (
            self.x_scale / 100.0,
            self.y_scale / 100.0,
            self.yaw_scale / 100.0,
        )
    ...
    def get_command(self, t):
        ...
        self.velocity_command[0] = (
            self.velocity_command[0] + accel_command[0] * self.x_scale
        )
        self.velocity_command[1] = (
            self.velocity_command[1] + accel_command[1] * self.y_scale
        )
        self.velocity_command[2] = (
            self.velocity_command[2] + accel_command[2] * self.yaw_scale
        )
        ...
        for button in range(4):
            if (
                ...
                and self.triggered_commands[button] is not None
            ):
...
class KeyboardProfile(CommandProfile):
    ...
    def __init__(self, dt, isaac_env, x_scale=1.0, y_scale=1.0, yaw_scale=1.0):
        ...
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, KeyboardInput.KEY_UP, "FORWARD"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, KeyboardInput.KEY_DOWN, "REVERSE"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, KeyboardInput.KEY_LEFT, "LEFT"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, KeyboardInput.KEY_RIGHT, "RIGHT"
        )
    ...
    def get_command(self, t):
        ...
        print(events_dict)
        if "FORWARD" in events_dict and events_dict["FORWARD"] == 1.0:
            self.keyb_command[0] = 1.0
        if "FORWARD" in events_dict and events_dict["FORWARD"] == 0.0:
            self.keyb_command[0] = 0.0
        if "REVERSE" in events_dict and events_dict["REVERSE"] == 1.0:
            self.keyb_command[0] = -1.0
        if "REVERSE" in events_dict and events_dict["REVERSE"] == 0.0:
            self.keyb_command[0] = 0.0
        if "LEFT" in events_dict and events_dict["LEFT"] == 1.0:
            self.keyb_command[1] = 1.0
        if "LEFT" in events_dict and events_dict["LEFT"] == 0.0:
            self.keyb_command[1] = 0.0
        if "RIGHT" in events_dict and events_dict["RIGHT"] == 1.0:
            self.keyb_command[1] = -1.0
        if "RIGHT" in events_dict and events_dict["RIGHT"] == 0.0:
            self.keyb_command[1] = 0.0
