...
from dreamwaq_direct.algorithms.cenet import CENet
...
class OnPolicyRunnerWaq(OnPolicyRunner):
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device: str = "cpu") -> None:
        ...
        # Augmented actor obs dim: base_obs + est_vel + context
        self.num_augmented_obs = self.num_base_obs + self.num_est_vel + self.num_context
        ...
        # Replace actor observation normalizer with Identity.
        # The original DreamWaQ code normalizes only base obs (45 dims) via its own RMS,
        # NOT the augmented obs (64 dims). The EmpiricalNormalization created during
        # __init__ was trained on zero-padded 64-dim obs, which gives near-zero variance
        # for the CENet dims and produces NaN when normalizing real data.
        self.alg.actor.obs_normalization = False
        self.alg.actor.obs_normalizer = torch.nn.Identity()
        # Normalize ONLY the base obs (45) with our own running mean/std, leaving est_vel(3) and
        # context(16) raw — this is what the original DreamWaQ does, and it also feeds the CENet
        # history. Without it the blind Base baseline (which keeps rsl-rl's normalizer) is
        # normalized while Waq is not, which handicaps Waq for reasons unrelated to the CENet.
        self.obs_rms = EmpiricalNormalization(shape=self.num_base_obs).to(device)
        # Normalize the CENet velocity target with its own running stats (DreamWaQ's
        # `true_vel_rms`); est_vel is then predicted in normalized-velocity space.
        ...
        # the chance of feeding the TRUE velocity (bootstrapping). Ramp it linearly 0 -> boot_max
        # over the first ``boot_ramp_frac`` of training so the actor is bootstrapped with ground
        # truth while the CENet is still untrained, then weaned onto est_vel.
        #
    ...
    def _register_std_clamp_hook(self):
        ...
        mean action was clamped to [-5, 0] and could never be positive. Measured in Manager across
        six runs: all 72 output biases were <= 0. Mirrors Manager.
        """
    ...
    def _get_flat_obs_history(self) -> torch.Tensor:
        ...
        The buffer stores RAW base obs; each frame is normalized with ``obs_rms`` at read time,
        matching the original DreamWaQ (which normalizes the CENet history input).
        """
    ...
    def _norm_base(self, base_obs: torch.Tensor) -> torch.Tensor:
        """Normalize the base obs (45) for the actor input (est_vel/context stay raw)."""
        return self.obs_rms(base_obs)

    def _build_actor_obs(self, base_obs, vel_input, context_vec) -> torch.Tensor:
        """Build the actor observation: [norm(base), vel, context] = 45 + 3 + 16 = 64."""
        actor_obs = torch.cat([self._norm_base(base_obs), vel_input, context_vec], dim=-1)
    ...
    def _extract_base_obs(self, obs) -> torch.Tensor:
        """Extract base observations (policy group) from TensorDict."""
        if hasattr(obs, '__getitem__') and "policy" in obs.keys():
            return obs["policy"]
        return obs

    def _extract_true_vel(self, obs) -> torch.Tensor:
        """Return the CENet's velocity-estimation target: the true (scaled) base LINEAR velocity.

        Uses the env's ``get_true_vel()`` (= base_lin_vel * obs_scale_lin_vel), mirroring the
        original DreamWaQ. Reading ``obs["critic"][:, :3]`` is WRONG for Base/Waq, where the
        critic group starts with angular velocity (lin vel is not observed by those variants),
        which made the CENet estimate angular instead of linear velocity.
        """
        return self.env.unwrapped.get_true_vel()
    ...
    def get_inference_policy(self, device: str | None = None):
        ...
        def policy(obs, dones=None):
            ...
            augmented = self._build_actor_obs(base_obs, est_vel, context_vec)
        ...
        for it in range(start_it, total_it):
            ...
            with torch.inference_mode():
                for _ in range(self.cfg["num_steps_per_env"]):
                    ...
                    # Actor obs: norm(base) + vel + context; est_vel/context stay raw
                    augmented_actor_obs = self._build_actor_obs(base_obs, vel_input, context_vec)
                    ...
                    # the recon target so the encoder input (normalized history) and the decoder
                    # target are on the SAME scale — matching the original DreamWaQ. Storing the
                    # raw obs here mis-scales recon_loss and poisons the CENet's learning signal.
                    ...
                    next_policy_obs = self._build_actor_obs(next_base_obs, next_est_vel, next_context_vec)
                    ...
                    # per-step timeout-rate nudge is gone so the schedule matches Manager.)
                ...
                final_augmented_obs["policy"] = self._build_actor_obs(
                    base_obs_final, est_vel_final, context_vec_final
                )
    ...
    def save(self, path: str, infos=None) -> None:
        ...
        saved_dict = {
            ...
            # key for loaders that already expect it, and add the stock key names so a Waq
            # checkpoint carries the same contents as a Base/Oracle one. Mirrors Manager.
            ...
            # base-obs running mean/std (normalizes actor base obs + CENet history) and the
            # velocity-target stats. Both MUST persist: a checkpoint restored without them
            # normalizes with fresh (zero-mean, unit-var) stats and the policy behaves nothing
            # like it did at save time.
        ...
        # This override skipped it. No-op for the tensorboard logger and before
        # ``init_logging_writer()``. Mirrors Manager.
    ...
    def load(self, path: str, load_optimizer: bool = True) -> None:
        ...
        # Load base-obs / velocity-target running mean-std
