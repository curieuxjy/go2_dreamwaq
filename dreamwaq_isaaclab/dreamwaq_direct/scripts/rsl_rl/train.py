"""Script to train DreamWaQ agent with RSL-RL (DirectRLEnv version).

Usage:
    python train.py --task=DreamWaQ-Direct-Go2-Base-v0 --headless
    python train.py --task=DreamWaQ-Direct-Go2-Base-v0 --headless --num_envs=64
"""
...
parser = argparse.ArgumentParser(description="Train DreamWaQ agent with RSL-RL (DirectRLEnv).")
...
def _safe_update(self, mlp_output: torch.Tensor) -> None:
    ...
    if self.std_type == "scalar":
        ...
        with torch.no_grad():
            self.log_std_param.data.clamp_(-5.0, 0.0)
            self.log_std_param.data = torch.nan_to_num(self.log_std_param.data, nan=0.0, posinf=0.0, neginf=-5.0)
...
def _register_std_clamp_for_all_runners(runner):
    ...
    # Measured in Manager across six runs: all 72 output biases were <= 0. The clamp itself
    # stays (std runaway / NaN guard); only its target is narrowed. Mirrors Manager.
    ...
    def _clamp_std_hook(optimizer, args, kwargs):
        for param in log_std_params:
            with torch.no_grad():
                param.data.clamp_(-5.0, 0.0)
                param.data = torch.nan_to_num(param.data, nan=0.0, posinf=0.0, neginf=-5.0)
