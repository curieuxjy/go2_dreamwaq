...
끝난다.
    ...
    python solution.py                    # 자산이 풀려 있으면 Waq 런들을 찾아 훑는다
    python solution.py --run <경로>        # run 폴더를 직접 준다 (여러 번 줄 수 있다)
"""
...
def diagnose_checkpoint(cenet_state_dict: dict, num_samples: int = 2048, seed: int = 0) -> dict:
    ...
    **j 번째 열 노름**에 전부 들어 있다. 열이 0 이면 그 입력은 뒷단에 아무 영향도 못 준다.
    """
    # ex:begin id=stage5-collapse-probe level=2 stage=stage5_compare inline_hints=2 task=CENet 체크포인트 하나에서 인코더 민감도와 디코더 사용량을 재고, 두 붕괴 기전을 구별한다
    ...
    #   hint: 인코더 프로브는 torch.randn(num_samples, INPUT_DIM, generator=g) — 표준정규다
    #   hint: 출력 35차원을 split([NUM_VEL, NUM_LATENT, NUM_LATENT], dim=-1) 로 est_vel / mu / logvar 로 가른다
    #   hint: KL(q||N(0,I)) = -0.5 * sum_j (1 + logvar_j - mu_j^2 - exp(logvar_j)) — 16차원으로 '합'하고 샘플로 평균낸다
    #   hint: est_vel_std 는 샘플 축(dim=0)의 표준편차를 구한 뒤 3차원 평균이다. 축을 바꾸면 뜻이 달라진다
    #   hint: 디코더 가중치 W 는 [hidden, 19] 다. 열 노름은 W.norm(dim=0) — 앞 3개가 v̂, 뒤 16개가 z 다
    #   hint: 판정 순서가 중요하다. NaN 을 먼저 걸러내고, |mu| 로 살았는지 보고, 죽었을 때만 |W_z|/|W_vel| 로 어느 기전인지 가른다
    enc = build_encoder(cenet_state_dict)
    gen = torch.Generator().manual_seed(seed)
    probe = torch.randn(num_samples, INPUT_DIM, generator=gen)

    with torch.no_grad():
        out = enc(probe)
    est_vel, mu, logvar = out.split([NUM_VEL, NUM_LATENT, NUM_LATENT], dim=-1)

    kl = (-0.5 * (1 + logvar - mu.pow(2) - logvar.exp())).sum(dim=-1).mean()

    col_norms = decoder_input_weight(cenet_state_dict).norm(dim=0)  # [19]
    dec_w_vel = col_norms[:NUM_VEL].mean().item()
    dec_w_z = col_norms[NUM_VEL:].mean().item()

    mu_abs = mu.abs().mean().item()
    if not (math.isfinite(mu_abs) and math.isfinite(dec_w_vel) and math.isfinite(dec_w_z)):
        # NaN 은 모든 부등호에서 False 라 그냥 두면 마지막 else 로 굴러떨어진다.
        mechanism = "nan"
    elif mu_abs >= MU_ALIVE:
        # 인코더가 살아 있으면 actor 가 받는 context 는 정보를 나른다. 디코더가 z 를 얼마나
        # 읽든 그것은 '앞으로 죽을 조짐'이지 이미 죽었다는 뜻이 아니다 — 그래서 mu 가 먼저다.
        mechanism = "alive"
    elif dec_w_z >= Z_USED * dec_w_vel:
        mechanism = "kl_overpressure"
    else:
        mechanism = "decoder_dropped_z"

    return {
        "mu_abs": mu_abs,
        "sigma": torch.exp(0.5 * logvar).mean().item(),
        "kl": kl.item(),
        "est_vel_std": est_vel.std(dim=0).mean().item(),
        "dec_w_vel": dec_w_vel,
        "dec_w_z": dec_w_z,
        "mechanism": mechanism,
    }
