...
같은 명령을 받는다. 결과가 왜 다른지가 이 실습의 핵심이다.
    ...
    python solution.py
    python solution.py --viz kit    # GUI 로 두 대를 나란히 보기
"""
...
def main() -> None:
    ...
    # ex:begin id=pd-target level=1 stage=stage1_isaac_basics task=action 으로부터 관절 목표를 계산한다
    ...
    def action_to_target(robot: Articulation, action: torch.Tensor) -> torch.Tensor:
        """정책 액션(±1)을 관절 위치 목표(rad)로 바꾼다."""
