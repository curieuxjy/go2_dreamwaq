...
def compute_gae(
    ...
    # ex:begin id=ppo-gae level=2 stage=stage3_ppo out=stage3_ppo/task01_gae/starter.py task=GAE 어드밴티지를 뒤에서 앞으로 누적해 계산한다
    #   hint: delta_t = r_t + gamma * V(s_{t+1}) * (1 - done_t) - V(s_t)  <- TD 오차
    #   hint: A_t = delta_t + gamma * lam * (1 - done_t) * A_{t+1}        <- 재귀
    #   hint: 시간 축을 거꾸로(reversed) 훑어야 A_{t+1} 을 이미 알고 있다
    #   hint: t = T-1 에서 V(s_{t+1}) 은 인자로 받은 last_value 다
    #   hint: done 인 스텝에서는 다음 상태로 이어지지 않으므로 (1 - done) 으로 끊는다
    #   hint: returns = advantages + values (critic 의 회귀 타깃)
...
def ppo_losses(
    ...
    """
    # ex:begin id=ppo-losses level=2 stage=stage3_ppo out=stage3_ppo/task02_ppo_update/starter.py task=PPO 의 clipped surrogate / value / entropy 손실과 총합을 완성한다
    ...
    total = surrogate + value_loss_coef * value_loss + entropy_coef * entropy_loss
    # ex:end
