...
class Curriculum:
    def set_to(self, low, high, value=1.0):
        inds = np.logical_and(
            self.grid >= low[:, None], self.grid <= high[:, None]
        ).all(axis=0)

        assert (
            len(inds) != 0
        ), "You are intializing your distribution with an empty domain!"
    ...
    def __init__(self, seed, **key_ranges):
        ...
        for key, v_range in key_ranges.items():
            ...
            cfg[key] = np.linspace(
                v_range[0] + bin_size / 2, v_range[1] - bin_size / 2, v_range[2]
            )
        ...
        self.bin_sizes = {
            key: (v_range[1] - v_range[0]) / v_range[2]
            for key, v_range in key_ranges.items()
        }
    ...
    def sample_bins(self, batch_size, low=None, high=None):
        ...
        if low is not None and high is not None:  # if bounds given
            ...
            inds = self.rng.choice(
                self.indices, batch_size, p=temp_weights / temp_weights.sum()
            )
        else:  # if no bounds given
            inds = self.rng.choice(
                self.indices, batch_size, p=self.weights / self.weights.sum()
            )
    ...
    def sample(self, batch_size, low=None, high=None):
        ...
        return (
            np.stack(
                [self.sample_uniform_from_cell(v_range) for v_range in cgf_centroid]
            ),
            inds,
        )
...
class RewardThresholdCurriculum(Curriculum):
    ...
    def update(self, bin_inds, task_rewards, success_thresholds, local_range=0.5):
        ...
        for adjacent in adjacents:
            ...
            self.weights[adjacent_inds] = np.clip(
                self.weights[adjacent_inds] + 0.2, 0, 1
            )
...
if __name__ == "__main__":
    ...
    assert r._raw_grid.shape == (3, 5, 2, 11), "grid shape is wrong: {}".format(
        r.grid.shape
    )
